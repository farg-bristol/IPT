...
/*To run on the mesh file*/
vector<vec<double,3>> Get_Coordinate_Vector(int &fin, size_t const &nPnts)
{
#ifdef DEBUG
    ...
    /*Convert it to a vector to store*/
    vector<vec<double,3>> coordVec(nPnts);
    for (uint ii = 0; ii < nPnts; ++ii)
    {
        coordVec[ii] = vec<double,3>(coordX[ii], coordY[ii], coordZ[ii]);
    }
    #ifdef DEBUG
